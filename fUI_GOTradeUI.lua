...
-- Trade tab UI builder (extracted from fUI_GOTrade.lua)

function LI.Trade.BuildTab_UI(depositPanel)
  ...
  modeBtn:SetPoint("TOP", leftPanel, "TOP", 0, -12)
  ...
  textArea:SetPoint("BOTTOM", leftPanel, "BOTTOM", 0, 130)
  ...
  btnChar:SetPoint("BOTTOM", leftPanel, "BOTTOM", -(BTN_W + BTN_GAP), ROW_Y)
  ...
  btnRealm:SetPoint("BOTTOM", leftPanel, "BOTTOM", (BTN_W + BTN_GAP), ROW_Y)
  ...
  btnAcc:SetPoint("BOTTOM", leftPanel, "BOTTOM", 0, ROW_Y)
  ...
  bankBtn:SetPoint("BOTTOM", leftPanel, "BOTTOM", -((BTN_W + BTN_GAP) / 2), ROW_Y + BTN_H + 2)
  ...
  guildTabBtn:SetPoint("BOTTOM", leftPanel, "BOTTOM", ((BTN_W + BTN_GAP) / 2), ROW_Y + BTN_H + 2)
  ...
  targetBox:SetPoint("BOTTOM", leftPanel, "BOTTOM", -((BTN_W + BTN_GAP) / 2), ROW_Y + BTN_H + 2)
  ...
  restockBtn:SetPoint("BOTTOM", leftPanel, "BOTTOM", ((BTN_W + BTN_GAP) / 2), ROW_Y + BTN_H + 2)
  ...
  actionBtn:SetPoint("BOTTOMLEFT", leftPanel, "BOTTOMLEFT", 10, 10)
  ...
  depositPanel._RefreshModeUI = function(self)
    ...
    if isDeposit then
      ...
      bankBtn:SetPoint("BOTTOMLEFT", leftPanel, "BOTTOMLEFT", 10, rowY)
      guildTabBtn:ClearAllPoints()
      guildTabBtn:SetPoint("BOTTOMRIGHT", leftPanel, "BOTTOMRIGHT", -10, rowY)
      ...
      keepScopeBtn:SetPoint("BOTTOMLEFT", leftPanel, "BOTTOM", -(totalW / 2), rowY)
  ...
  -- Items list (Trade tab): permanent right-side list; lets you delete rules without having the item.
  do
    ...
    itemPop:SetPoint("TOPLEFT", rightPanel, "TOPLEFT", 0, 0)
    itemPop:SetPoint("BOTTOMRIGHT", rightPanel, "BOTTOMRIGHT", 0, 0)
    ...
    local function UpdateContentWidth()
      ...
      local w = tonumber(scroll:GetWidth() or 0) or 0
      if w < 1 then w = 1 end
      content:SetWidth(w)
    end
    ...
    if scroll and scroll.HookScript then
      scroll:HookScript("OnSizeChanged", function() UpdateContentWidth() end)
    end
    ...
    if itemPop and itemPop.HookScript then
      itemPop:HookScript("OnShow", function()
        UpdateContentWidth()
        if type(BuildItemsList) == "function" then
          BuildItemsList()
        end
      end)
      itemPop:HookScript("OnSizeChanged", function()
        UpdateContentWidth()
      end)
    end
      ...
      BuildItemsList = function()
        ...
        if mode == "deposit" then
          ...
          if t == "guild" then
            ...
            local isRandom = GetRandomFlag(cfg, rk)
            if isRandom then
              ctxLabel = "Guild: Random"
            else
              local v = GetGuildTabValue(cfg, rk)
              ctxLabel = (v <= 0) and "Guild: Current" or ("Guild: Tab " .. tostring(v))
            end
          elseif t == "personal" then
            ctxLabel = "Personal"
          elseif t == "warbank" then
            ctxLabel = "Warbank"
          else
            ctxLabel = "Bank"
          end
        ...
        if ctxLabel then
          subtitle:SetText(modeLabel .. " (" .. ctxLabel .. ") rules: " .. tostring(#list))
        else
        ...
        for i = 1, #list do
          ...
          if not row then
            ...
            -- If the list was previously empty, row #1 can exist as a placeholder row
            -- with only row.txt. Upgrade it to a full row before we try to use
            -- row.lvl/hp/mp/edit/del.
            row:SetHeight(rowH)

            if row.txt then
              if row.txt.SetFontObject then
                row.txt:SetFontObject("GameFontHighlightSmall")
              end
              row.txt:SetJustifyH("LEFT")
              row.txt:SetWordWrap(false)
            else
