...
LI.Mail = LI.Mail or {}
  ...
  local function MailDebugEnabled()
    local env = Mail._notifierEnv or {}
    SafeCall(env.EnsureDB)
    local db = SafeCall(env.GetDB)
    if type(db) == "table" and type(db.mailNotify) == "table" then
      return (db.mailNotify.debug == true)
    end
    return false
  end

  local function MailDebugPrint(line)
    if not MailDebugEnabled() then return end
    local env = Mail._notifierEnv or {}
    local PrintFn = env.Print or LI.Print or function(...) end
    SafeCall(PrintFn, "Mail debug: " .. tostring(line or ""))
  end
  ...
  -- Slash handler for /fli mail ... (kept in Mail module for portability).
  function Mail.HandleSlash(rest, env)
    if type(env) == "table" then
      Mail.SetNotifierEnv(env)
    end
    ...
    local CreateConfigUI = env.CreateConfigUI or function(...) return nil end

    local MailNotifyCfg = env.MailNotifyCfg or function(...) return nil end
    local UpdateMailNotifier = env.UpdateMailNotifier or Mail.UpdateMailNotifier or function(...) end
    local CreateMailNotifier = env.CreateMailNotifier or Mail.CreateMailNotifier or function(...) return nil end
    local ApplyMailModelToFrame = env.ApplyMailModelToFrame or Mail.ApplyMailModelToFrame or function(...) end

    SafeCall(EnsureDB)
    local DB = SafeCall(GetDB)
    local CHARDB = SafeCall(GetCharDB)
    local v = (tostring(rest or "")):lower()

    if DB then DB.mailNotify = DB.mailNotify or {} end

    local function GetMailDebugCfg()
      -- Debug is account-wide regardless of mail scope.
      if type(DB) ~= "table" then return nil end
      if type(DB.mailNotify) ~= "table" then DB.mailNotify = {} end
      return DB.mailNotify
    end

    if v:match("^debug") then
      local _, arg = v:match("^(debug)%s*(%S*)")
      arg = tostring(arg or ""):lower()

      local cfg = GetMailDebugCfg()
      if type(cfg) ~= "table" then return end

      if arg == "" or arg == "status" then
        SafeCall(Print, "Mail debug: " .. ((cfg.debug == true) and "on" or "off"))
        return
      end
      if arg == "on" or arg == "1" or arg == "true" then
        cfg.debug = true
        SafeCall(Print, "Mail debug: on")
        return
      end
      if arg == "off" or arg == "0" or arg == "false" then
        cfg.debug = false
        SafeCall(Print, "Mail debug: off")
        return
      end
      if arg == "toggle" then
        cfg.debug = not (cfg.debug == true)
        SafeCall(Print, "Mail debug: " .. ((cfg.debug == true) and "on" or "off"))
        return
      end

      SafeCall(Print, "Usage: /fgo li mail debug on|off|toggle|status")
      return
    end
    ...
    if v:match("^model") then
      ...
      elseif kind == "display" then
        local n = tonumber(id)
        if not n then
          SafeCall(Print, "Usage: /fgo li mail model display <id>")
          return
        end
      ...
      elseif kind == "npc" or kind == "creature" then
        local n = tonumber(id)
        if not n then
          SafeCall(Print, "Usage: /fgo li mail model npc <id>")
          return
        end
        mnc.model.kind = "npc"
        mnc.model.id = n
      ...
      elseif kind == "file" then
        local n = tonumber(id)
        if not n then
          SafeCall(Print, "Usage: /fgo li mail model file <id>")
          return
        end
        mnc.model.kind = "file"
        mnc.model.id = n
        ...
        SafeCall(Print, "Usage: /fgo li mail model [player|npc <id>|display <id>|file <id>]")
        return
      end
    end
    ...
    SafeCall(Print, "Usage: /fgo li mail on|acc|off|toggle|test")
  end
  ...
  Mail._bootstrapTicker = Mail._bootstrapTicker or nil

  function Mail.SafeUpdateMailNotifier()
    if type(Mail.UpdateMailNotifier) ~= "function" then return end
    pcall(Mail.UpdateMailNotifier)
  end

  function Mail.BootstrapMailNotifier()
    ...
    if not (C_Timer and C_Timer.NewTicker and C_Timer.After) then
      MailDebugPrint("bootstrap (no C_Timer): update now")
      Mail.SafeUpdateMailNotifier()
      return
    end

    -- If a bootstrap loop is already running, just force an update now.
    if Mail._bootstrapTicker then
      MailDebugPrint("bootstrap: already running; force update")
      Mail.SafeUpdateMailNotifier()
      return
    end

    MailDebugPrint("bootstrap: start")
    ...
    local function Tick()
      ...
      if not stop then
        ...
        if has then
          local mn = Mail.GetMailNotifier and Mail.GetMailNotifier() or nil
      ...
      if stop and Mail._bootstrapTicker and Mail._bootstrapTicker.Cancel then
        Mail._bootstrapTicker:Cancel()
        Mail._bootstrapTicker = nil
        MailDebugPrint("bootstrap: stop")
      end
    end
  ...
  function Mail.OnMailboxInteraction(isOpen)
    MailDebugPrint("interaction: mailbox " .. ((isOpen == true) and "open" or "closed"))
    Mail.SetMailboxOpen(isOpen == true)
    if isOpen == true then
      Mail.SafeUpdateMailNotifier()
      return
    end
    if C_Timer and C_Timer.After then
      C_Timer.After(0.40, Mail.BootstrapMailNotifier)
    else
      Mail.BootstrapMailNotifier()
    end
  end

  function Mail.OnPendingMailChanged()
    MailDebugPrint("refresh: pending mail changed")
    Mail.BootstrapMailNotifier()
  end

  function Mail.OnCombatStateChanged()
    local inCombat = (UnitAffectingCombat and UnitAffectingCombat("player")) and true or false
    MailDebugPrint("combat: inCombat=" .. tostring(inCombat))
    Mail.SafeUpdateMailNotifier()
  end
  ...
  function LI.Mail.BuildTab(mailPanel, mailUI, env)
    ...
    local function GetMailNotifyCfgForSelectedScope()
      EnsureDB()
      local scope = tostring(GetMailNotifyScope() or "acc"):lower()
      if scope == "char" or scope == "character" then
        local charDB = GetCharDB()
        if type(charDB) ~= "table" then
          local ok, res = pcall(MailNotifyCfg)
          if ok then return res end
          return nil
        end
      ...
      if type(db.mailNotify) ~= "table" then
        db.mailNotify = {}
      end
      return db.mailNotify
    end

    local function GetMailDebugCfgAcc()
      -- Debug is account-wide regardless of mail scope.
      EnsureDB()
      local db = GetDB()
      if type(db) ~= "table" then return nil end
    ...
    mailUI.mailScopeBtn:SetPoint("TOPLEFT", mailPanel, "TOPLEFT", 10, 12)
    ...
    local function RefreshMailDebugButton()
      local btn = mailUI and mailUI.debugBtn
      if not (btn and btn.SetText) then return end
      local cfg = GetMailDebugCfgAcc()
      local on = (type(cfg) == "table") and (cfg.debug == true) or false
      if on then
        -- Match UIPanelButtonTemplate default label color (same as Reload UI button).
        btn:SetText("Debug")
      else
        btn:SetText("|cff808080Debug|r")
      end
    end

    -- Debug toggle (mail notifier only). Controls the "Mail debug:" chat spam.
    mailUI.debugBtn = CreateFrame("Button", nil, mailPanel, "UIPanelButtonTemplate")
    mailUI.debugBtn:SetSize(90, 22)
    if reloadBtn then
      mailUI.debugBtn:SetPoint("BOTTOMRIGHT", reloadBtn, "BOTTOMLEFT", -6, 0)
    else
      mailUI.debugBtn:SetPoint("LEFT", mailUI.mailScopeBtn, "RIGHT", 10, 0)
    end
    mailUI.debugBtn:SetScript("OnClick", function()
      local cfg = GetMailDebugCfgAcc()
      if type(cfg) ~= "table" then return end
      cfg.debug = not (cfg.debug == true)
      RefreshMailDebugButton()
      pcall(Print, "Mail debug: " .. ((cfg.debug == true) and "on" or "off"))
    end)
    RefreshMailDebugButton()

    -- Embedded mail model editor (replaces the old pop-out window).
    local modelUI = CreateFrame("Frame", nil, mailPanel)
    modelUI:SetPoint("TOPLEFT", mailUI.mailScopeBtn, "BOTTOMLEFT", -2, -2)
    ...
    mailUI.combatBtn:SetScript("OnClick", function()
      local mn = GetMailNotifyCfgForSelectedScope()
      if not mn then return end
      local rawBefore = tostring((type(mn) == "table") and mn.showInCombat or nil)
      local beforeEff = true
      if type(mn) == "table" then
        beforeEff = (mn.showInCombat ~= false)
      end
      mn.showInCombat = not beforeEff

      local rawImmediate = tostring((type(mn) == "table") and mn.showInCombat or nil)
      local immediateEff = true
      if type(mn) == "table" then
        immediateEff = (mn.showInCombat ~= false)
      end

      UpdateMailNotifier()

      local dbg = GetMailDebugCfgAcc()
      if type(dbg) == "table" and dbg.debug == true then
        local rawPostUpdate = tostring((type(mn) == "table") and mn.showInCombat or nil)
        local postUpdateEff = true
        if type(mn) == "table" then
          postUpdateEff = (mn.showInCombat ~= false)
        end
        pcall(Print, string.format(
          "Mail debug: combat toggle: scope=%s, rawBefore=%s, before=%s, raw=%s, eff=%s, postUpdateRaw=%s, postUpdate=%s",
          tostring(GetMailNotifyScope() or "acc"),
          tostring(rawBefore),
          tostring(beforeEff),
          tostring(rawImmediate),
          tostring(immediateEff),
          tostring(rawPostUpdate),
          tostring(postUpdateEff)
        ))
      end
      RefreshMailCombatButton()
    end)
  ...
  -- Notify the FGO host popout (if present) that Mail UI is ready to build.
  do
    local notify = _G and rawget(_G, "FGO_MailModuleReady")
