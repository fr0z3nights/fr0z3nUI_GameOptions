...
function LI.LootTab.BuildTab(lootPanel, env)
  ...
  local function CommitCombineCount(self, clearFocus)
    EnsureDB()
    local DB = GetDB()
    if not DB then return end
    ...
    if clearFocus and self.ClearFocus then
      self:ClearFocus()
    end
  ...
  combineBox:SetScript("OnEnterPressed", function(self)
    CommitCombineCount(self, true)
  end)

  combineBox:SetScript("OnEditFocusLost", function(self)
    CommitCombineCount(self, false)
  end)

  combineBox:SetScript("OnEscapePressed", function(self)
    EnsureDB()
    local DB = GetDB()
    if not DB then return end
    self:SetText(tostring(DB.lootCombineCount or 1))
    self:ClearFocus()
  end)
  ...
  local function RefreshCombineModeButtons()
    ...
    if modeToggle._text then
      modeToggle._text:SetText(isLoot and "Loot Window" or "Print Per")
  ...
  modeToggle:SetScript("OnClick", function()
    ...
    local isLoot = (tostring(DB.lootCombineMode or "loot") == "loot")
    DB.lootCombineMode = (isLoot and "per" or "loot")
    LootCombineCancelTimers()
    LootCombineFlush()
  ...
  local function CommitPrefix(self, clearFocus)
    EnsureDB()
    local DB = GetDB()
    if not DB then return end
  ...
  reset:SetScript("OnClick", function()
    ...
    if type(AutoGame_Acc) == "table" and type(AutoGame_Acc.lootIt) == "table" then
      ClearTableInPlace(AutoGame_Acc.lootIt)
    else
      fr0z3nUI_LootItDB = {}
    end

    if type(AutoGame_Char) == "table" and type(AutoGame_Char.lootIt) == "table" then
      ClearTableInPlace(AutoGame_Char.lootIt)
    else
